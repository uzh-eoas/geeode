...
Another data-structure implication is that the number of images in *the original collection* does not necessarily equal the number of actual observations in *each pixel cell*.

If the timestamps for each image are then recorded alongside the observation's value (i.e., within an array image), each pixel grid then has an individual time series recorded at the pixel level (with missing data values, that of course can be removed). Pixel grid cells in the array-image (before sampling) will then, often as a result, have varying numbers of observations and the structure could be described as "ragged".
...
- can turn "ragged" pixel grid cells into time series of uniform length (without masked observations / NA value "holes")

### Background

The realities of Earth Observation are that datasets are often "ragged" and, moreover, there are often real-world factors that determine the quality of input data.

For example with satellite imagery, clouds frequently contaminate parts of scenes rendering certain pixel locations unusable, and in certains areas clouds exhibit seasonal patterns (e.g., "wet" or "rainy" seasons will often have more clouds than "dry" seasons). Therefore, datasets not only exhibit hetereogeneity in missing values *across a spatial scale* but also *across a temporal scale*.
...
- At every point, it's possible to calcuate the number of points neighboring it in time(consider "kernel density" methods).
...
To understand the sampling methods offered, it's important to understand how simple random sampling works conceptually:

- Given any dataset, it's possible to sample data points by assigning a random numeric value between 0 and 1 (exclusive);
- Then, order the points by these random numbers
- Lastly, remove `x` number of points sequentially from *one end* of the newly ordered series of datapoints (i.e., either the points with the highest or lowest numeric value).

This approach to random sampling has the opportunity of allowing arithmetic operations on the numeric values used to sort the data.

Namely, one can multiply (i.e., scale) each random numeric value assigned to each data point by a weight value. After doing so, data points with higher weight values will have a greater chance of being sorted *on one end* (the higher end) of the sorted data.

For temporal subsampling within GEEODE, the temporal density for each point within a specified time kernel is used as the scaling weight. The higher a point's temporal density is, the greater the chance it will have a higher numeric value when sorting (and thus the greater the likelihood it will be dropped).
...
Order the observations by temporal density and remove `x` numbers of observations starting with the highest and moving towards the lowest such that `n` desired number of observations is maintained.

!["Bulk" Simple Weighted Sampling](./graphics/bulk_sampling.png)

#### "Leapfrog" Sampling

Sort the observations by temporal density (descending, such that the observations with the highest temporal density come last). Beginning with the last observation, remove every other observation until `n` desired number of observations is achieved.

!["Leapfrog" Sampling](./graphics/leapfrog_sampling.png)

#### "Split-Shuffle" Sampling

Point by point, "split-shuffle":

- Sort by temporal density
- Assign every observation to `s` number of groups (in the following plot, illustrated by color)
- Order all groups by their original assignment order
- Within the groups, order individually by temporal density (descending)
- Starting from the end, remove `x` number of observations until the desired number of observations is maintained.
